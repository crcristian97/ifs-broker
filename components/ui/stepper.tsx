...
function Stepper({
  ...
  const registerTrigger = React.useCallback((node: HTMLButtonElement | null) => {
    ...
    setTriggerNodes((prev) => {
      ...
      return next.sort(
        (a, b) =>
          Number(a.dataset.stepperStep ?? 0) - Number(b.dataset.stepperStep ?? 0),
      );
    });
  }, []);
  ...
  const focusNext = (currentIdx: number) =>
    focusTrigger((currentIdx + 1) % Math.max(triggerNodes.length, 1));
  const focusPrev = (currentIdx: number) =>
    focusTrigger((currentIdx - 1 + triggerNodes.length) % Math.max(triggerNodes.length, 1));
  const focusFirst = () => focusTrigger(0);
  const focusLast = () => focusTrigger(Math.max(triggerNodes.length - 1, 0));
...
function StepperTrigger({ asChild = false, className, children, tabIndex, ...props }: StepperTriggerProps) {
  ...
  const { setActiveStep, activeStep, registerTrigger, unregisterTrigger, triggerNodes, focusNext, focusPrev, focusFirst, focusLast } =
    stepperCtx;
  ...
  }, [asChild, registerTrigger, unregisterTrigger]);
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLButtonElement>) => {
    switch (e.key) {
      ...
      case "ArrowDown":
        e.preventDefault();
        if (myIdx !== -1 && triggerNodes.length) focusNext(myIdx);
        break;
      case "ArrowLeft":
      case "ArrowUp":
        e.preventDefault();
        if (myIdx !== -1 && triggerNodes.length) focusPrev(myIdx);
        break;
      case "Home":
        e.preventDefault();
        if (triggerNodes.length) focusFirst();
        break;
      case "End":
        e.preventDefault();
        if (triggerNodes.length) focusLast();
        break;
      case "Enter":
      case " ":
        e.preventDefault();
...
function StepperIndicator({ children, className }: React.ComponentProps<"div">) {
  ...
  return (
    <div
      ...
      data-state={state}
      className={cn(
        "relative flex size-6 shrink-0 items-center justify-center overflow-hidden rounded-full border border-background bg-accent text-xs text-accent-foreground data-[state=completed]:bg-primary data-[state=completed]:text-primary-foreground data-[state=active]:bg-primary data-[state=active]:text-primary-foreground",
        className,
      )}
    >
